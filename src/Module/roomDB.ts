...
function leave(data) {
  ...
  const UpdatePlayer = player.filter(
    (value) => value.nickname != data.nickname
  );
...
function score(data) {
  const Room = Roomdb.get("RoomData").find({ _id: data[0] }).value();
  const RoomP = [];
...
function password(data) {
  const password = Roomdb.get("RoomData").find({ _id: data }).value().password;
